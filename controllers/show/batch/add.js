'use strict';

const { pool } = require('../../../db');
const { generateBatchId } = require('../../../utils/generateId');

module.exports = async (req, res) => {
    try {
        const { event_id, name, price, capacity } = req.body;

        if (!event_id || !name || price == null || capacity == null) {
            return res.status(400).json({ success: false, error: 'Required fields missing' });
        }

        // შემოწმება Event-ის არსებობისთვის
        const [eventRows] = await pool.query('SELECT id FROM show_event WHERE id = ?', [event_id]);
        if (!eventRows.length) return res.status(404).json({ success: false, error: 'Event not found' });

        const id = generateBatchId();

        const sql = `
      INSERT INTO show_batch 
        (id, event_id, name, price, capacity, sold_count, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, 0, NOW(), NOW())
    `;

        await pool.query(sql, [id, event_id, name, price, capacity]);

        res.json({ success: true, id, message: 'Batch created successfully' });
    } catch (err) {
        console.error(err);
        res.status(500).json({ success: false, error: 'Server error' });
    }
};
