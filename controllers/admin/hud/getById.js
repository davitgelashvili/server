    ...
    try {
        ...
        const [rows] = await pool.query(
            `SELECT h.id, h.title, h.slug, h.description, h.cover, h.start_datetime, h.end_datetime,
             ...
             FROM show_hud h WHERE h.id = ?`,
            [id]
        );

        if (!rows.length) return res.status(404).json({ success: false, message: 'HUD not found' });

        const hud = rows[0];
        res.json({ success: true, hud: { ...hud, description: hud.description || '' } });
    } catch (err) {
        console.error(err);
