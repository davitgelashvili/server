...
module.exports = async (req, res) => {
    try {
        ...
        const [hudRows] = await pool.query('SELECT id FROM show_hud WHERE id = ?', [id]);
        if (!hudRows.length) return res.status(404).json({ success: false, message: 'HUD not found' });
        ...
        await pool.query('DELETE FROM show_hud WHERE id = ?', [id]);
